...
use xtables_server::value::Value;
use xtables_server::websocket::message::ControlMessage;
...
pub(crate) const NO_DATA_SENTINEL: &str = "XTABLES_INTERNAL_NO_DATA_AVAILABLE";
/// The WebSocket topic the server relays log lines on.
pub(crate) const LOG_TOPIC: &str = "XTABLES_INTERNAL_LOG";
...
pub(crate) fn decode_xtables_type(tag: i32, data: &[u8]) -> Option<supported_values::Kind> {
...
///
/// ```no_run
/// use xtables_client::client::Client;
///
...
pub struct Client {
    ...
    pub(crate) logger: std::sync::OnceLock<xtables_protobuf::wpilog::Logger>,
}
...
impl Client {
    ...
    ///
    /// ```no_run
    /// # use xtables_client::{Client, Value};
    /// # let client = Client::new();
