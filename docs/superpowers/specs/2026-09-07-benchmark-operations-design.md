...
- Round-trip cases for the Java XTABLES harness. The catalog can express them
...
| `implementations` | which of `xtables-rust`, `ntcore`, `xtables` can run it |
| `mode` | `Delivery` or `RoundTrip`, defined below |
| `params` | payload sizes and publish rate |

The initial catalog:

| Case | Group | Mode | xtables-rust | ntcore | xtables |
|---|---|---|---|---|---|
...
{"schema": 1,
 ...
 "implementations": {"xtables-rust": "0.1.0", "ntcore": "2027.0.0a6.post4"},
 "cases": [{"case": "publish", "group": "delivery", "mode": "delivery",
            "impl": "xtables-rust", "payload_bytes": 96,
...
| Today | Becomes |
|---|---|
| `xtables-rust` | `publish`, implementation `xtables-rust` |
...
| `xtables` | `publish`, through the Java client |
