...
impl Probe {
    ...
    /// `XTablesClient`, and pinning starves them against the pacer's spin.
...
pub(crate) fn plan(case: &str, implementation: &str) -> Plan {
    ...
    match (case, implementation) {
        ...
        ("publish", "xtables") => Plan {
            server: Server::Xtables,
...
///
/// XTABLES is started with `java -jar`, the invocation its release notes give;
...
pub(crate) fn server_command(
    ...
    match server {
        ...
        Server::Xtables => Some((
            "java".to_string(),
            vec!["-jar".into(), env.xtables_jar.clone()?],
        )),
    }
}
