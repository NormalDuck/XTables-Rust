...
//! gets, publishing through `XTablesClient` rather than straight onto a socket.
//!
...
//! is the ordinary NT4 one, so the only difference from `xtables-rust` is who
//! writes the value.

use crate::harness::{HEADER_LEN, Pacer, SendStats, encode, now_nanos};
use std::time::Instant;
use xtables_client::XTablesClient;
...
pub fn publish(host: &str, payload: usize, rate_hz: u64, count: u64) -> std::io::Result<()> {
    ...
    let client = XTablesClient::connect(host);
