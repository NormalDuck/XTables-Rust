...
impl Conditions {
    ...
    pub fn sample() -> Self {
        Conditions {
            ...
            warmup: 500,
            reps: 3,
            implementations: vec!["xtables-rust=0.1.0".to_string()],
        }
    }
...
fn blurb(group: &str) -> &'static str {
    match group {
        "servers" => {
            "Each server with its client library taken out of the path. `xtables-rust` and `ntcore` \
are driven by the same raw NT4 publisher from this repo; `xtables` speaks ZeroMQ, so it is driven \
by a raw JeroMQ publisher sending the bytes its own client would. That row carries a JVM where the \
other two carry a Rust process, so it is a ceiling on the XTABLES server; read it against \
`xtables` in the clients table, the same JVM and wire with `XTablesClient` added back."
        }
        "clients" => {
...
mod tests {
    ...
    fn the_matrix_has_a_cell_for_every_case_and_implementation() {
        let records = vec![
            record("publish", "xtables-rust", 34.1),
    ...
    fn an_implementation_without_a_record_reads_as_absent() {
        let records = vec![record("publish", "xtables-rust", 71.2)];
        let out = markdown(&Conditions::sample(), &records);
        assert!(
    ...
    fn a_win_inside_the_run_to_run_range_is_called_noise() {
        ...
        let mut ours = record("publish", "xtables-rust", 36.03);
    ...
    fn an_unknown_release_or_cpu_is_left_out_rather_than_printed() {
        ...
        let out = markdown(&bare, &[record("publish", "xtables-rust", 34.1)]);
    ...
    fn a_malformed_row_is_rejected_with_its_line_number() {
        let short = "ROW\tpublish\txtables-rust\t0.1.0\t96\t34.10\t10.0\t20.0";
