...
subscriber from this repo, so the only thing that differs is which server answered. `xtables` \
...
library, which for `ntcore` and `xtables` is the only way to speak their protocols at all."
        }
        "round-trip" => {
...
mod tests {
    ...
    fn the_matrix_has_a_cell_for_every_case_and_implementation() {
        let records = vec![
            ...
            record("get", "xtables-rust-client", 71.2),
        ];
    ...
    fn an_implementation_without_a_record_reads_as_absent() {
        let records = vec![record("get", "xtables-rust-client", 71.2)];
        let out = markdown(&Conditions::sample(), &records);
        assert!(
            out.contains('-'),
    ...
    fn a_malformed_row_is_rejected_with_its_line_number() {
        let short = "ROW\txtables-rust\t96\t34.10\t10.0\t20.0\t25.0\t30.0";
