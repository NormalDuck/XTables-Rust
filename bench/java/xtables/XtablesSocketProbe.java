...
import org.kobe.xbot.Utilities.Entities.XTableProto.XTableMessage;
...
/**
 * The XTABLES server driven straight over its own ZeroMQ sockets, with
 * XTablesClient taken out of the path.
 *
 * XTablesClient.publish does not touch the wire: it serializes the message and
 ...
 * instead, so the difference between this row and the XTablesClient row is that
 ...
 * its bytecode: one PUSH frame carrying a serialized XTableMessage, one SUB
 * frame carrying a serialized XTableUpdate.
 */
public final class XtablesSocketProbe implements Probe {
    ...
    private static byte[] encode(String key, byte[] value) {
        return XTableMessage.newBuilder()
            .setKey(key)
            .setCommand(XTableMessage.Command.PUBLISH)
    ...
    public void subscribe(String host, int payload, int samples) throws Exception {
        ...
        try (ZContext context = new ZContext()) {
            ...
            while (collected.size() < samples && System.currentTimeMillis() < deadline) {
                ...
                XTableUpdate update = XTableUpdate.parseFrom(frame);
