...
run_client() {
  ...
  BENCH_LABEL="xtables-rust client v$(cargo pkgid -p xtables_client 2>/dev/null | sed 's/.*[#@]//' | sed 's/.*://')" \
...
table_for() {
  ...
  rows="$(awk -F'\t' -v p="$1" -v class="$2" '
    $3 == p {
      ...
      else if ($2 ~ /^xtables-rust v/) { row = "reliable" }
      else { row = "client" }
      if (row != class) next
...
RESULTS="$ROOT/bench/RESULTS.md"
{
  ...
  for pay in $PAYLOADS; do
    ...
    echo "client library, which for \`ntcore\` and \`xtables\` is the only way to"
