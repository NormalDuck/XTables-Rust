...
for pay in $PAYLOADS; do
  ...
  has xtables-rust && { echo "rep $rep payload ${pay}B: xtables-rust" >&2; attempt xtables-rust run_rust_nt4 "$pay"; }
  ...
  if [ "$JAVA_OK" = "1" ]; then
    has xtables    && { echo "rep $rep payload ${pay}B: xtables" >&2;      attempt xtables      run_xtables_java "$pay"; }
  fi
done
done
fi
...
RESULTS="$ROOT/bench/RESULTS.md"
{
  ...
  for pay in $PAYLOADS; do
    ...
    if table_for "$pay" client > /dev/null; then
      ...
      echo "which is what a robot writes against. \`xtables\` appears only here:"
