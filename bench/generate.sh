...
run_telemetry() {
  ...
  wait $sub; capture "$out" "telemetry_publish" "xtables-rust"; stop_server
}
...
run_client() {
  ...
  BENCH_LABEL="xtables-rust v$(cargo pkgid -p xtables_client 2>/dev/null | sed 's/.*[#@]//' | sed 's/.*://')" \
  ...
  wait $sub; capture "$out" "publish" "xtables-rust-client"; stop_server
}
...
run_rust_nt4() {
  ...
  nohup $PIN_SERVER "$SERVER" >/dev/null 2>&1 & SERVER_PID=$!
  bench_own_port "$SERVER_PID" t 5810 || { stop_server; return 1; }
  BENCH_LABEL="xtables-rust server v$(cargo pkgid -p xtables_server 2>/dev/null | sed 's/.*[#@]//' | sed 's/.*://')" \
    timeout "$LIMIT" $PIN_SUB "$B" subscriber --subject nt4 --payload "$pay" --samples "$SAMPLES" > "$out" 2>&1 &
  local sub=$!
  timeout "$LIMIT" $PIN_PUB "$B" publisher --subject nt4 --payload "$pay" --rate "$RATE" --count "$COUNT" > "$ROWS/nt4_pub_${pay}_r${REP:-1}.log" 2>&1
  wait $sub; capture "$out" "publish" "xtables-rust"; stop_server
}
...
run_xtables_java() {
  ...
  nohup $PIN_SERVER java -cp "$BENCH_XTABLES_JAR" org.kobe.xbot.JServer.Main > "$ROWS/xtj_server_${pay}_r${REP:-1}.log" 2>&1 & SERVER_PID=$!
  ...
  sleep 5
  timeout "$LIMIT" $PIN_PUB java -cp "$BENCH_CP" xtables.Main publisher --subject xtables-java --payload "$pay" --rate "$RATE" --count "$COUNT" > "$ROWS/xtj_pub_${pay}_r${REP:-1}.log" 2>&1
  wait $sub; capture "$out" "$case_name" "xtables"; stop_server
}
...
run_case() {
  ...
  case "$implementation" in
    ...
    xtables)
      [ "$JAVA_OK" = "1" ] || return 0
      run_xtables_java "$case_name" "$pay"
      return
      ;;
  esac
  ...
  case "$mode" in
    ...
    delivery)
      case "$implementation" in
        xtables-rust | xtables-rust-client)
          nohup $PIN_SERVER "$SERVER" >/dev/null 2>&1 & SERVER_PID=$!
          bench_own_port "$SERVER_PID" t 5810 || { stop_server; return 1; }
          ;;
      esac
