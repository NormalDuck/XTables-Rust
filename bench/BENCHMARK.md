...
`bench sweep` runs `./gradlew benchEnv` itself to resolve the XTABLES release jar
...
against a current one. Without a JDK the `xtables` implementation is skipped and
the rest still run.
...
|---|---|---|
| `publish` | servers | `xtables-rust`, `ntcore`, `xtables` |
| `publish_client` | clients (rendered as `publish`) | `xtables-rust`, `ntcore`, `xtables` |
...
calls. That is the Java `XTablesClient` for `xtables`, pyntcore for `ntcore`,
and this repo's client for `xtables-rust`. It renders as `publish` in the
...
`xtables` serves ZeroMQ instead — PUSH 48800, REQUEST 48801, SUBSCRIBE 48802 —
so its servers row is driven by `XtablesSocketProbe`, written against those
sockets with the JeroMQ the XTABLES jar bundles. It sends the bytes their own
client sends, `XTableMessage{key, command=PUBLISH, value}` in one PUSH frame,
and reads one SUB frame per `XTableUpdate`; no registration is needed.

That row carries a JVM where the other two carry a Rust process, so it is a
ceiling on the XTABLES server rather than a like-for-like number. Read it
against `xtables` in the clients table, which is the same JVM and sockets with
`XTablesClient` added back; the difference is that library, whose `publish`
...
It runs as three processes, the same shape as `xtables-rust`: a server of its own
...
`ntcore` is driven from Python and `xtables` from Java, because their protocols
...
JeroMQ does. So does `XTablesClient`, whose `publish` hands the message to a
...
once. Measured on this machine at 500 Hz, 96 B, the same xtables `publish` case
...
`XTablesClient`, which runs a pool, still reads 546.82 us and drops 2.91 per
...
siblings left to the kernel throughout. Pinned to one core, the XTABLES JVM
dropped one to two per cent of messages; on its own set of cores it drops none.

XTABLES does not drop messages at these rates. Every loss figure earlier
