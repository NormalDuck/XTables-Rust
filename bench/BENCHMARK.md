...
| `client` | the same path as `xtables-rust`, published through the Rust client rather than onto a socket |
...
and `xtables` all publish through their project's own library, which is what a
robot's code actually calls. For `ntcore` and `xtables` there was never another
option, since their protocols are only reachable through their stacks; the
`xtables` publisher is the Java `XTablesClient` and the `ntcore` one is pyntcore.
Our Java and Python clients belong in this table too when someone wires them up.

**Transport, no library** holds `xtables-rust`, the same server driven straight
...
through `XTablesClient` instead, and the difference is the library.
