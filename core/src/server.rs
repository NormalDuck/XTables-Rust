...
mod tests {
    ...
    fn value_frame(pubuid: u32, value: XtValue) -> Vec<u8> {
        ...
        crate::websocket::message::ValueMessage {
            topic_id: pubuid,
            timestamp_micros: XTablesServer::now_micros(),
    ...
    fn a_value_an_nt_client_writes_is_readable_over_the_control_plane() {
        let server = XTablesServer::with_ports_and_telemetry(22301, 22302, 22303, 22304);
        server.start();
    ...
    fn a_value_of_the_wrong_type_reaches_neither_plane() {
        let server = XTablesServer::with_ports_and_telemetry(22311, 22312, 22313, 22314);
        server.start();
        std::thread::sleep(Duration::from_millis(200));
    ...
    fn the_server_stops_accepting_past_the_connection_cap() {
        ...
        let server = XTablesServer::with_ports_and_telemetry(22321, 22322, 22323, 22324);
        server.start();
        std::thread::sleep(Duration::from_millis(200));
...
mod telemetry_registration_tests {
    ...
    fn register(server: &XTablesServer, socket: &std::net::UdpSocket, hash: u32, to: SocketAddr) {
    ...
    fn one_datagram_is_not_amplified_past_the_subscriber_cap() {
        ...
        let server = XTablesServer::with_ports_and_telemetry(22201, 22202, 22203, 22204);
        server.start();
    ...
    fn a_subscriber_keeps_its_slot_when_the_channel_is_full() {
        ...
        for port in 0..MAX_TELEMETRY_SUBSCRIBERS as u16 {
            assert!(XTablesServer::register_telemetry(
                &registry,
                &published,
                7,
                address(9000 + port)
        ...
        assert!(
            !XTablesServer::register_telemetry(&registry, &published, 7, address(9999)),
            "a full channel has to turn a new address away"
        );
        assert!(
            XTablesServer::register_telemetry(&registry, &published, 7, address(9000)),
